...
from config import APP_NAME, MALAYSIA_EMERGENCY_NUMBER, NPRA_PRODUCT_SEARCH_URL, OPENAI_API_KEY
from core.ai_assistant import ai_available, analyze_image, ask_guardian, transcribe_audio
...
def sidebar() -> str:
    ...
    if not OPENAI_API_KEY:
        st.sidebar.warning("OpenAI key not configured. Core offline modules still work.")
...
def dashboard_page() -> None:
    ...
    if ai_available() and azure_voice_available():
        status_cols[2].success("AI, live search, image and Azure voice: ready")
    elif ai_available():
        status_cols[2].warning("AI ready; add Azure Speech secrets for spoken replies")
    else:
        status_cols[2].warning("AI features: add OPENAI_API_KEY")
...
def ai_page() -> None:
    hero(
        "🤖 AI Assistant",
        "Ask general questions, use live search, record a voice question, and hear natural Azure Neural replies.",
    )
    ...
    live_search = col1.toggle(
        "Use live web search",
        value=True,
        help="Recommended for current or changing information.",
    )
    speak = col2.toggle(
        "Generate natural spoken reply",
        value=False,
        help="Uses Azure Neural TTS instead of OpenAI speech.",
    )
    ...
    if audio and st.button("Transcribe and ask voice question"):
        ...
        if transcript.lower().startswith("transcription failed") or "requires openai" in transcript.lower():
            st.error(transcript)
        else:
